...
class WireGuard(plugins.Plugin):
    ...
    def _connect(self):
        ...
        # We don't care if this fails (it will if the interface doesn't exist).
        try:
            subprocess.run(["wg-quick", "down", self.wg_config_path], capture_output=True)
        except FileNotFoundError:
            # wg-quick isn't installed, so we can't do anything.
...
"""
    ...
    def on_handshake(self, agent, filename, access_point, client_station):
        if self.ready and self.status == "Connected":
            ...
            command = ["scp", "-o", "StrictHostKeyChecking=no", "-o", "BatchMode=yes", filename, f"{server_user}@{server_ip}:{remote_path}"]
